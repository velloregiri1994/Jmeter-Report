...
def build_comparison_insights(baseline_comparison: Optional[Dict[str, Any]]) -> str:
    ...
    if regressions:
        ...
        reg_details = []
        for r in sorted(regressions, key=lambda x: (x.get("delta_p95") or 0.0, x.get("delta_error") or 0.0), reverse=True)[:5]:
            label = r["label"]
            dp = r.get("delta_p95")
            de = r.get("delta_error")
            parts = []
...
def build_overall_insights(global_stats: Dict[str, Any],
    ...
    if global_stats["errors"] == 0:
        items.append("No HTTP-level errors were observed; focus tuning efforts on latency and capacity rather than stability.")
    else:
...
def build_html(metrics: Dict[str, Any],
    ...
    ts_labels = [datetime.fromtimestamp(ts["sec"]).strftime("%Y-%m-%d %H:%M:%S") for ts in timeseries]
