#!/usr/bin/env python3
"""
JMeter HTML Performance Dashboard (Tabs, Filters, Advanced Analysis) - v3.7
--------------------------------------------------------------------

Enhanced Features in v3.7:
...
# =========================
# CONTAINER COMPARISON ANALYSIS (NEW in v3.7)
# =========================
...
def build_enhanced_overall_insights(global_stats: Dict[str, Any],
    ...
    """
    Enhanced overall insights with health score, root causes, and prioritized recommendations.
    """
    items: List[str] = []

    # Calculate health score
    health_score = calculate_health_score(global_stats, sla_result, metrics)
    capacity = estimate_capacity(metrics)

    # Health Score Display
    items.append(f"""
    <div class="health-score-card" style="background: linear-gradient(90deg, {health_score['grade_color']} {health_score['overall']}%, #e5e7eb {health_score['overall']}%); 
           padding: 12px; border-radius: 8px; margin: 10px 0; color: white;">
        <div style="font-size: 14px; font-weight: 600;">Overall Health Score</div>
        <div style="font-size: 28px; font-weight: 700; margin: 4px 0;">{health_score['overall']}/100 <span style="background: rgba(255,255,255,0.2); padding: 2px 8px; border-radius: 12px; font-size: 16px;">{health_score['grade']}</span></div>
        <div style="font-size: 12px; opacity: 0.9;">{health_score['interpretation']} • SLA: {health_score['components']['sla']:.1f} • Errors: {health_score['components']['errors']:.1f} • Latency: {health_score['components']['latency_stability']:.1f} • Throughput: {health_score['components']['throughput_consistency']:.1f}</div>
    </div>
    """)
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 8px; margin: 12px 0;">
        ...
        <div class="summary-item">
            ...
            <div class="summary-value {'good' if global_stats['error_rate'] < 0.01 else 'warning' if global_stats['error_rate'] < 0.05 else 'critical'}">{global_stats['error_rate'] * 100:.2f}%</div>
        </div>
        <div class="summary-item">
            <div class="summary-label">P95 Latency</div>
            <div class="summary-value {'good' if global_stats['p95_ms'] < 1000 else 'warning' if global_stats['p95_ms'] < 3000 else 'critical'}">{global_stats['p95_ms']:.0f} ms</div>
        </div>
        <div class="summary-item">
            <div class="summary-label">Peak Throughput</div>
            <div class="summary-value">{metrics['peak_rps']:.1f} req/sec</div>
        </div>
    </div>
    """)
    ...
    if sla_result["overall_status"]:
        ...
        <div class="status-banner success">
            ...
            <strong>SLA Compliance: PASS</strong> ({sla_result['passed_checks']}/{sla_result['total_checks']} checks passed)
        </div>
        """)
    else:
        items.append(f"""
        <div class="status-banner critical">
            <span style="font-size: 18px;">❌</span>
            <strong>SLA Compliance: FAIL</strong> ({sla_result['passed_checks']}/{sla_result['total_checks']} checks passed)
        </div>
        """)

    # Capacity Assessment
    items.append(f"""
    <div class="capacity-card" style="border-left: 4px solid {capacity['capacity_color']}; background-color: rgba(255,255,255,0.5); padding: 10px; margin: 12px 0;">
        <div style="font-weight: 600; color: {capacity['capacity_color']};">Capacity Assessment: {capacity['capacity_grade']}</div>
        <div style="font-size: 12px; margin-top: 4px;">
            Sustainable: ~{capacity['estimated_sustainable_rps']:.1f} RPS • 
            Concurrent Users: ~{capacity['estimated_concurrent_users']} • 
            Peak Handled: {capacity['peak_handled']:.1f} RPS
        </div>
        {f"<div style='font-size: 11px; color: #dc2626; margin-top: 4px;'>Bottlenecks: {', '.join([b['issue'] for b in capacity['bottlenecks']][:2])}</div>" if capacity['bottlenecks'] else ""}
    </div>
    """)

    # Root Cause Analysis
    root_causes = analyze_root_causes(metrics, sla_result, infra_metrics)
    if root_causes:
        items.append("<div style='font-weight: 600; margin: 16px 0 8px 0;'>🔍 Probable Root Causes</div>")
        for cause in root_causes[:3]:  # Show top 3
            items.append(f"""
            <div class="root-cause" style="border-left: 3px solid {cause['confidence_color']}; padding: 8px 0 8px 12px; margin: 6px 0;">
                <div style="display: flex; align-items: center; gap: 8px;">
                    <span>{cause.get('icon', '•')}</span>
                    <div style="font-weight: 500; color: {cause['confidence_color']};">{cause['title']}</div>
                    <div style="font-size: 10px; background: {cause['confidence_color']}20; color: {cause['confidence_color']}; padding: 1px 6px; border-radius: 10px;">{cause['confidence'].upper()}</div>
                </div>
                <div style="font-size: 12px; color: #4b5563; margin-top: 4px;">{cause['description']}</div>
                <div style="font-size: 11px; color: #6b7280; margin-top: 4px; font-style: italic;">Suggested: {cause['remediation']}</div>
            </div>
            """)
    ...
    if recommendations["critical"]:
        items.append(
            "<div class='priority-header critical' style='color: #dc2626; font-weight: 600; margin: 20px 0 8px 0;'>🚨 CRITICAL ACTION REQUIRED</div>")
        for rec in recommendations["critical"]:
            items.append(f"""
            <div class="recommendation critical">
                <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 6px;">
                    <span>{rec.get('icon', '⚠️')}</span>
                    <div style="font-weight: 500;">{rec['title']}</div>
                </div>
                <div style="font-size: 12px; color: #4b5563; margin-bottom: 8px;">{rec['description']}</div>
                <div style="font-size: 11px;">
                    <div style="font-weight: 500; margin-bottom: 4px;">Action Items:</div>
                    {"".join([f"<div style='margin-left: 16px; margin-bottom: 2px;'>• {item}</div>" for item in rec.get('action_items', [])[:3]])}
                </div>
            </div>
            """)
    ...
    if recommendations["high"]:
        items.append(
            "<div class='priority-header high' style='color: #ea580c; font-weight: 600; margin: 16px 0 8px 0;'>⚠️ HIGH PRIORITY</div>")
        for rec in recommendations["high"][:2]:
            items.append(f"""
            <div class="recommendation high">
                ...
                <div style="font-size: 12px; color: #4b5563;">{rec['description']}</div>
            </div>
            """)
    ...
    if total_recs > 0:
        items.append(f"""
        <div style="font-size: 11px; color: #6b7280; margin-top: 12px; padding: 8px; background: #f9fafb; border-radius: 6px;">
            <strong>Recommendation Summary:</strong> {len(recommendations['critical'])} critical, {len(recommendations['high'])} high, {len(recommendations['medium'])} medium, {len(recommendations['low'])} low priority
        </div>
        """)

    # Test outcome verdict
    if sla_result["overall_status"] and global_stats["error_rate"] < 0.05 and health_score["overall"] >= 70:
        items.append("""
        <div class="verdict success" style="background-color: #d1fae5; padding: 12px; border-radius: 8px; margin: 16px 0; border: 1px solid #86efac;">
            <div style="display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 24px;">✅</span>
                <div>
                    <div style="font-weight: 700; color: #065f46;">VERDICT: TEST PASSED</div>
                    <div style="font-size: 12px; color: #047857;">System meets performance requirements and is ready for production load.</div>
                </div>
            </div>
        </div>
        """)
    elif not sla_result["overall_status"] or health_score["overall"] < 60:
        items.append("""
        <div class="verdict critical" style="background-color: #fee2e2; padding: 12px; border-radius: 8px; margin: 16px 0; border: 1px solid #fca5a5;">
            <div style="display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 24px;">❌</span>
                <div>
                    <div style="font-weight: 700; color: #b91c1c;">VERDICT: TEST FAILED</div>
                    <div style="font-size: 12px; color: #dc2626;">System does not meet SLA requirements. Address critical issues before proceeding.</div>
                </div>
            </div>
        </div>
        """)
        ...
        items.append("""
        <div class="verdict warning" style="background-color: #fef3c7; padding: 12px; border-radius: 8px; margin: 16px 0; border: 1px solid #fcd34d;">
            <div style="display: flex; align-items: center; gap: 10px;">
                <span style="font-size: 24px;">⚠️</span>
                <div>
                    <div style="font-weight: 700; color: #92400e;">VERDICT: TEST PASSED WITH CONCERNS</div>
                    <div style="font-size: 12px; color: #b45309;">SLAs met but performance issues need attention before production deployment.</div>
                </div>
            </div>
        </div>
        """)
    ...
    if recommendations["critical"] or recommendations["high"]:
        items.append("""
        <div style="font-weight: 600; margin: 16px 0 8px 0; color: #374151;">🎯 Recommended Next Steps</div>
        <div style="font-size: 12px; padding-left: 20px;">
            <div style="margin-bottom: 4px;">1. Address critical recommendations immediately</div>
            <div style="margin-bottom: 4px;">2. Review detailed metrics in respective tabs</div>
            <div style="margin-bottom: 4px;">3. Create action plan with assigned owners</div>
            <div>4. Schedule follow-up test after fixes</div>
        </div>
        """)
...
<div class="container">
  ...
  <div class="header">
    <h1>JMeter Performance Report v3.7</h1>
    <div class="meta">
...
def main():
    ...
    parser.add_argument("--output", help="Output HTML report path (required unless --excel-only)")
    ...
    parser.add_argument("--infra-json", nargs="*",
                        help="Optional JSON files with infrastructure metrics (CPU/Memory for load/soak tests).")
    ...
    # Excel Export
    # If you have Excel export function, update it too:
        ...
        if not EXCEL_SUPPORT:
            ...
            try:
                excel_path = export_to_excel(
                    ...
                    container_comparison=container_comparison,  # Pass the container_comparison
    ...
    if not args.excel_only:
        ...
        print(f"✓ Enhanced HTML report v3.7 generated: {args.output}")
        print(f"  - Health Score calculation with letter grade")
        print(f"  - Root Cause Analysis with confidence levels")
        print(f"  - Container comparison (Before/During/After)")
        ...
        if container_comparison:
            print(f"  - Container resource analysis included")
        if args.excel_export:
