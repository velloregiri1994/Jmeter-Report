...
# -------------------------------------------------------------------------
# DEFAULT SYSDIG CONFIG (fallback if no YAML provided)
# -------------------------------------------------------------------------
DEFAULT_SYSDIG = {
    "api_url": "https://app.sysdigcloud.com/prometheus/api/v1",
    "api_token": "bf82af7a-0c05-43ac-94c1-1f3419119d95",  # ← UPDATE YOUR TOKEN HERE
    "cluster": "stage-eu-central-1-rg8mt",
    "namespace": "boarding-bcs-ppe",
    "step": "10",
    "workloads": [
...
def load_config(config_path: Optional[str], project: Optional[str]) -> Dict[str, Any]:
    """
    Load YAML configuration and extract settings for the selected project.
    Returns a dict with sysdig and jmeter settings.
    """
    if not config_path:
        return {}
    ...
    project_config = full_config.get("PROJECT_CONFIG", {})
    sysdig_cfg = full_config.get("sysdig", {})
    jmeter_cfg = full_config.get("jmeter", {})

    # If a project is specified, use its settings
    if project and project in project_config:
        proj = project_config[project]
        sysdig_cfg["cluster"] = proj.get("cluster", sysdig_cfg.get("cluster"))
        sysdig_cfg["namespace"] = proj.get("namespace", sysdig_cfg.get("namespace"))
        sysdig_cfg["workloads"] = proj.get("workloads", sysdig_cfg.get("workloads", []))
    elif project:
        logging.warning(f"Project '{project}' not found in PROJECT_CONFIG; using global defaults")

    return {"sysdig": sysdig_cfg, "jmeter": jmeter_cfg}
...
# ---------------------------------------------------------
# SYSDIG JSON PARSING (legacy file support)
# ---------------------------------------------------------
...
def workloads_regex_from_list(workloads: List[str]) -> str:
    """Build a regex that matches any workload in the list (exact match after stripping rollout suffix)."""
    if not workloads:
        return r"(?!)"
...
def discover_workloads_from_sysdig(
    ...
    Filter by patterns:
      - If use_regex=False: exact match after stripping rollout suffix.
      - If use_regex=True: patterns are regex (e.g. '^bcs-.*').
    """
    if not patterns:
        logging.warning("No workload patterns provided; returning empty list")
        return []
    ...
    try:
        ...
        payload = resp.json()
        if payload.get("status") != "success":
            logging.warning("Sysdig label discovery did not return success; using fallback patterns")
            return patterns if not use_regex else []
        ...
        for wl in all_workloads:
            if use_regex:
                ...
                if any(re.search(pattern, wl) for pattern in patterns):
                    discovered.append(wl)
            else:
                # Strip rollout suffix and check exact match
                base = re.sub(r'-[0-9]+$', '', wl)
                if base in patterns and wl not in discovered:
                    discovered.append(wl)
        ...
        if discovered:
            ...
            logging.warning("No matching workloads found in Sysdig; using fallback patterns")
            return patterns if not use_regex else []

    except Exception as e:
        logging.warning(f"Sysdig discovery failed ({e}); using fallback patterns")
        return patterns if not use_regex else []
...
def main():
    ...
    parser.add_argument("--config", help="Path to YAML configuration file (overrides defaults)")
    parser.add_argument("--project", help="Project name to use from PROJECT_CONFIG (e.g., BCS, BIS, POLARIS)")
    parser.add_argument("--load-test", help="Path to JMeter load test CSV")
    parser.add_argument("--soak-test", help="Path to JMeter soak test CSV")
    parser.add_argument("--template", help="Path to Word template")
    parser.add_argument("--output", help="Output Word document path")

    args = parser.parse_args()

    # Load YAML config if provided (using the corrected load_config function)
    config = load_config(args.config, args.project) if args.config else {}

    # Merge config with command-line arguments (command-line takes precedence)
    sysdig_cfg = {**DEFAULT_SYSDIG, **config.get("sysdig", {})}
    jmeter_cfg = config.get("jmeter", {})

    # Determine JMeter file paths: CLI args > YAML config > (no default, required)
    load_test_path = args.load_test or jmeter_cfg.get("load_test_csv")
    soak_test_path = args.soak_test or jmeter_cfg.get("soak_test_csv")
    template_path = args.template or jmeter_cfg.get("template_docx")
    output_path = args.output or jmeter_cfg.get("output_docx")

    # Validate required inputs
    if not load_test_path or not soak_test_path or not template_path or not output_path:
        parser.error("Missing required inputs. Provide via --config YAML or command-line arguments.")

    # Sysdig API config
    sysdig_api_url = sysdig_cfg["api_url"].rstrip("/")
    sysdig_api_token = sysdig_cfg["api_token"]
    sysdig_cluster = sysdig_cfg["cluster"]
    sysdig_namespace = sysdig_cfg["namespace"]
    sysdig_step = sysdig_cfg.get("step", "10")

    if not sysdig_api_token:
        raise RuntimeError("Sysdig API token is required. Provide via YAML or update DEFAULT_SYSDIG.")

    # Workload discovery: patterns or explicit list
    workload_patterns = sysdig_cfg.get("workload_patterns")
    explicit_workloads = sysdig_cfg.get("workloads")
    ...
    if workload_patterns:
        ...
        use_regex = True
    elif explicit_workloads:
        patterns = explicit_workloads
        use_regex = False
    else:
        # Fallback to hardcoded default
        patterns = DEFAULT_SYSDIG["workloads"]
        use_regex = False

    # Discover workloads from Sysdig
    ...
    load_records = parse_jmeter_csv(Path(load_test_path))
    soak_records = parse_jmeter_csv(Path(soak_test_path))
